...
def run_fetch_job():
    ...
    try:
        ...
        fetchers = [
            ("RSS", RssFetcher()),
            ("PubMed", PubMedFetcher(journal_list=JOURNAL_LIST))
        ]
        ...
        # We process each fetcher strategy
        for fetcher_name, fetcher in fetchers:
            logger.info(f"--- Starting Strategy: {fetcher_name} ---")
            for journal in JOURNAL_LIST:
                try:
                    ...
                    # Log Success
                    log_repo.create_log(journal, f"success-{fetcher_name}", new_papers_count)
                    logger.info(f"Journal {journal} ({fetcher_name}): {new_papers_count} new papers ingested.")
                    
                except Exception as e:
                    # Log Failure
                    logger.error(f"Failed to process journal {journal} via {fetcher_name}: {e}")
                    log_repo.create_log(journal, f"failure-{fetcher_name}", 0, str(e))
